...
const SignIn = () => {
  ...
  const adminEmailRoutes = {
    'admin@gmail.com': '/R_AdminDashboard',
  ...
  const getRedirectUrl = (email, userType) => {
    ...
    if (userType === 'admin') {
      return adminEmailRoutes[normalizedEmail] || '/R_AdminDashboard';
    }

    return '/StudentProfile';
  };

  const storeAuthenticatedUser = (userData, token, email, successMessage, redirectUserType) => {
    ...
    navigate(getRedirectUrl(email, redirectUserType));
    return true;
  };

  const createLocalAdminSession = (email) => {
    const normalizedEmail = email.toLowerCase().trim();
    ...
    return storeAuthenticatedUser(
      {
        id: `local-admin-${normalizedEmail}`,
      ...
      '',
      normalizedEmail,
      `Welcome ${displayName}! Admin access is ready.`,
      'admin'
    );
  };

  const loginWithBackend = async (email, password, expectedUserType) => {
    ...
    if (expectedUserType && authenticatedUser.userType !== expectedUserType) {
      throw new Error(`This account is not registered as an ${expectedUserType}.`);
    }

    return storeAuthenticatedUser(
      {
        ...authenticatedUser,
        userType: authenticatedUser.userType || expectedUserType || 'student'
      },
      response.data.token,
      email,
      `Welcome ${authenticatedUser.fullName || 'User'}! Login successful.`,
      authenticatedUser.userType || expectedUserType || 'student'
    );
  };
  ...
  const handleStudentLogin = async (email, password) => {
    try {
      return await loginWithBackend(email, password, 'student');
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (userType === 'admin') {
          loginSuccess = await handleAdminLogin(email, password);
          if (!loginSuccess) {
            setServerError('Unable to start the admin session. Please check your details.');
          }
        } else {
