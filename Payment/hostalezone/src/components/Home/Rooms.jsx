...
const RoomsPage = () => {
  ...
  useEffect(() => {
    const loadRoomPaymentPreview = async () => {
      if (!selectedRoom) {
        setPaymentPreview(null);
        return;
      }

      if (!token || currentUser.userType !== 'student') {
        setPaymentPreview(null);
        return;
      }
      ...
      try {
        const response = await getPaymentPreview({
          ...
          block: selectedRoom.roomNumber?.split('-')[0] || ''
        });
        setPaymentPreview(response.data.preview);
      } catch (err) {
        console.error('Error loading room payment preview:', err);
        setPaymentPreview(null);
      } finally {
    ...
    loadRoomPaymentPreview();
  }, [selectedRoom, token, currentUser.id, currentUser.studentId, currentUser.userType]);

  useEffect(() => {
    if (currentUser?.fullName || currentUser?.email || currentUser?.phone) {
      setBookingForm((current) => ({
        ...current,
        name: current.name || currentUser.fullName || '',
                  ...
                  <div className="sticky top-8">
                    ...
                    <div className="bg-white rounded-xl p-5 mb-6 shadow-sm border border-blue-100">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <h4 className="font-semibold text-gray-900">Fee-Inclusive Estimate</h4>
                          <p className="text-sm text-gray-500 mt-1">
                            Monthly total with current bill, water, and related fees.
                          </p>
                        </div>
                        <svg className="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 14h6m-6-4h6m2 10l-2-1-2 1-2-1-2 1-2-1-2 1V4l2-1 2 1 2-1 2 1 2-1 2 1v16z" />
                        </svg>
                      </div>
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-3 mt-4 text-sm">
                            <div className="rounded-xl bg-blue-50 p-3">
                              ...
                              <p className="font-semibold text-gray-900">{formatCurrency(paymentPreview.pricing?.roomPrice)}</p>
                            </div>
                            <div className="rounded-xl bg-blue-50 p-3">
                              <p className="text-gray-500">Current Bill</p>
                              <p className="font-semibold text-gray-900">{formatCurrency(paymentPreview.pricing?.currentBill)}</p>
                            </div>
                            <div className="rounded-xl bg-blue-50 p-3">
                              <p className="text-gray-500">Water Bill</p>
                              <p className="font-semibold text-gray-900">{formatCurrency(paymentPreview.pricing?.waterBill)}</p>
                            </div>
                            <div className="rounded-xl bg-blue-50 p-3">
                              <p className="text-gray-500">Other Fees</p>
                              <p className="font-semibold text-gray-900">{formatCurrency(paymentPreview.pricing?.additionalFeesTotal)}</p>
                            </div>
                          </div>
                          <div className="mt-4 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 px-4 py-3 text-white flex items-center justify-between">
                            <span className="text-sm text-blue-100">Estimated total</span>
                            <span className="text-xl font-bold">{formatCurrency(paymentPreview.totalPrice)}</span>
                          </div>
                        </>
                      ) : (
                        <div className="mt-4 rounded-xl bg-blue-50 p-4 text-sm text-blue-700">
                          {token && currentUser.userType === 'student'
                            ? 'Pricing preview is not available for this room yet.'
                            : 'Sign in as a student to see the full monthly fee estimate for this room.'}
                        </div>
                      )}
                    </div>
